...
Use Elite when OwnID owns the primary hosted auth UI and the app owns the entry
point, provider setup, native registration/session handoff, lifecycle, and
fallback UI.
...
Register app-wide providers through top-level `OwnID.setProviders`. Use
`OwnID.flows.withProviders` when one Elite run needs provider behavior different
from the app-wide bindings.
...
`start` on the returned derived handle. Prefer one-run context for shared login
...
- Do not abort Elite Flow from `SwiftUI.onDisappear`; presenting the SDK-hosted
  UI may trigger it.
- Keep the controller and settlement task in a dedicated lifecycle-aware owner,
  such as an `ObservableObject`, rather than directly in the view.

Interpret `whenSettled()` as infrastructure settlement:

- `.success`: a terminal hosted-page handler completed. This can follow
  `onFinish`, `onNativeAction`, `onError`, or `onClose`; it does not mean hosted
  authentication succeeded. Create the app session from the applicable
  `onFinish` or `onNativeAction` handoff.
...
For SDK-managed WebView container customization, use:

- `backgroundColor`

Set options when the app needs SDK-managed WebView container or safe-area
background customization. Confirm exact option behavior in the selected
release source/API comments.
