...
Choose WebBridge when the page needs native OwnID SDK capabilities such as
...
Resolve trusted origins, relying-party domains, provider setup, backend session
contracts, and wildcard-origin policy before attaching the bridge.

## Providers And Context

Register shared providers through top-level `OwnID.setProviders` for
provider-backed capabilities the page can call. Use scoped `withProviders` when
this `WKWebView` session needs behavior different from the top-level bindings.
...
Use the WebBridge docs for the capability table and origin rules. Use
the providers, context, and namespace-handle references for their detailed
contracts.

## Origins And Attachment

Attach to reviewed trusted origins. Pass explicit `allowedOriginRules` when
server config may not be available yet or when the session needs a
local/tenant-specific trusted origin. Treat `*` as a production
...
Most apps use the default plugin set. Configure per-bridge custom plugins
through `bridge.plugins` before `attach(...)`, using fresh plugin instances per
bridge. Configure `OwnID.webBridge.defaultPluginFactories` before `create()` so
new bridges receive the intended defaults.
