...
struct CoreAPIContractTests {
    ...
    // Covers: WIRE-260
    @Test func `API unexpected mapping collapses transport status decode and body-shape failures`() async throws {
        let call = TestUnexpectedAPICall()
        let request = NetworkRequest(url: baseURL.appendingPathComponent("unexpected"))

        let transport = try requireFailure(
            try await call.execute(
                network: APIRecordingNetwork(
                    response: .fail(
                        .networkError(.init(url: baseURL, error: URLError(.notConnectedToInternet)))
                    )
                ),
                request: request
            )
        )
        try assertUnexpected(transport, errorCode: .network)

        let status = try requireFailure(call.mapHttpSuccess(.init(url: baseURL, code: 202, headers: [:], body: "")))
        try assertUnexpected(status, errorCode: .unknown)

        let decode = try requireFailure(call.mapHttpSuccess(.init(url: baseURL, code: 200, headers: [:], body: "{")))
        try assertUnexpected(decode, errorCode: .unknown)

        let bodyShape = try requireFailure(call.mapHttpSuccess(.init(url: baseURL, code: 200, headers: [:], body: "{}")))
        try assertUnexpected(bodyShape, errorCode: .unknown)
    }
...
private enum TestUnexpectedAPIFailure: APIFailure, CustomStringConvertible {
    ...
    var description: String {
        "unexpected(errorCode: \(errorCode))"
    }
}

private struct TestUnexpectedAPICall: APICall {
    struct Body: Decodable {
        let value: String
    }

    let request = NetworkRequest(url: URL(string: "https://example.test/unexpected")!)
    private let coder = JSONCoderImpl()

    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<String, TestUnexpectedAPIFailure> {
        do {
            guard successResponse.code == 200 else {
                return .failure(mapUnhandled(successResponse.toUnexpectedStatusFail()))
            }

            let body = try coder.decodeFromString(successResponse.body, as: Body.self)
            guard !body.value.isEmpty else {
                return .failure(mapUnhandled(successResponse.toSuccessMappingFail(message: "Malformed test response")))
            }
            ...
            return .failure(
                mapUnhandled(
                    successResponse.toSuccessMappingFail(message: "Failed to parse test response", cause: error)
                )
            )
        }
    }

    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> TestUnexpectedAPIFailure {
        failResponse.toUnexpectedFailure(TestUnexpectedAPIFailure.unexpected)
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> TestUnexpectedAPIFailure {
        failResponse.toUnexpectedFailure(TestUnexpectedAPIFailure.unexpected)
    }
}

private func assertUnexpected(
    _ failure: TestUnexpectedAPIFailure,
    errorCode: ErrorCode,
    sourceLocation: SourceLocation = SourceLocation(fileID: #fileID, filePath: #filePath, line: #line, column: #column)
) throws {
    guard case .unexpected(let actualErrorCode, _, let underlyingError) = failure else {
        return try #require(nil as Void?, "Expected unexpected failure, got \(failure)", sourceLocation: sourceLocation)
    }

    #expect(actualErrorCode == errorCode, sourceLocation: sourceLocation)
    #expect(underlyingError is APIUnexpectedError, sourceLocation: sourceLocation)
}
