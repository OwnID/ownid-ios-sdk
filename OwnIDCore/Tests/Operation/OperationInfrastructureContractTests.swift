...
struct OperationInfrastructureContractTests {
    ...
    @Test func `Direct API entry starts only while operation entry owns availability preflight`() async throws {
        let apiRecorder = APIEntryRecorder()
        let apiContainer = DIContainerImpl(scopeName: "api-entry-start-only")
        apiContainer.register((any TestAPIRuntime).self, instance: RecordingAPIRuntime(recorder: apiRecorder))
        ...
        #expect(try requireSuccess(await apiEntry.start(params: TestAPIParams(value: "direct"))) == "api:direct")
        #expect(apiRecorder.values() == ["direct"])

        let operationRecorder = OperationEntryRecorder()
        let operationEntry = makeOperationEntry(recorder: operationRecorder)

        try requireAvailable(await operationEntry.availability(params: TestOperationParams(value: "preflight")))
        _ = try #require(operationEntry.start(params: TestOperationParams(value: "launch")) as? RecordingOperationController)
...
private struct TestAPIParams: Sendable {
    let value: String
}
...
private final class RecordingAPIRuntime: TestAPIRuntime, @unchecked Sendable {
    private let recorder: APIEntryRecorder

    init(recorder: APIEntryRecorder) {
        self.recorder = recorder
    }
...
private final class APIEntryRecorder: @unchecked Sendable {
    private let lock = NSLock()
