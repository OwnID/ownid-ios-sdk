...
final class FlowOperationBox<Params: CapabilityParams, Success: Sendable, Failure: OperationFailure>: @unchecked Sendable {
    ...
    private(set) var startedControllers: [FlowOperationControllerFake<Success, Failure>] = []
    ...
    func makeController(
        ...
        let (result, operationID) = lock.withLock {
            startParams.append(params)
            scopedContexts.append(context)
            let result = startResults.count > 1 ? startResults.removeFirst() : startResults[0]
            return (result, OperationID(type: operationType, id: UUID().uuidString))
        }

        let controller = FlowOperationControllerFake<Success, Failure>(operationID: operationID, result: result)
        lock.withLock { startedControllers.append(controller) }
        return controller
    }
}

final class FlowOperationControllerFake<Success: Sendable, Failure: OperationFailure>: OperationController, @unchecked Sendable {
    private let controller: OperationControllerImpl<Success, Failure>
    let abortReasons: FlowLocked<[Reason]>

    var operationID: OperationID { controller.operationID }

    init(operationID: OperationID, result: OperationResult<Success, Failure>) {
        let abortReasons = FlowLocked<[Reason]>([])
        self.abortReasons = abortReasons
        controller = OperationControllerImpl<Success, Failure>(operationID: operationID) { reason in
...
final class FlowSessionCreateFake: SessionCreate, @unchecked Sendable {
    ...
    init(available: Bool = true, session: (any Sendable)? = "host-session") {
        ...
        self.createResult = .success(SessionOutput(session: session))
    }

    init(available: Bool = true, createResult: Result<SessionOutput, any Error & Sendable>) {
        self.available = available
