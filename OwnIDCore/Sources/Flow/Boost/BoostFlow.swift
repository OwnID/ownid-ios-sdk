...
extension BoostFlowCreatePasskeyResponse {
    /// Returns ``ownIdData`` when `currentLoginID` matches this response login ID.
    ///
    /// The supplied `currentLoginID` is trimmed before comparison. `nil`, blank, whitespace-only, and non-matching
    /// values return `nil`. Use this helper when the app keeps a create-passkey response for later registration submit
    /// and the current form login ID may have changed since the response was emitted.
    public func ownIdData(forLoginID currentLoginID: String?) -> String? {
        let normalizedLoginID = currentLoginID?.trimmingCharacters(in: .whitespacesAndNewlines)

        guard let normalizedLoginID, !normalizedLoginID.isEmpty, normalizedLoginID == loginID.id else {
            return nil
        }
