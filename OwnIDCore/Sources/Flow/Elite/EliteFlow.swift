...
/// handler returns, including hosted `onError` and `onClose` events. This success result means the hosted terminal
/// event was handled; it does not mean hosted authentication succeeded. ``FlowResult/failure(_:)`` is reserved for SDK
/// or WebBridge failures, such as a missing WebBridge operation or a WebView-level failure. ``FlowResult/canceled(_:)``
...
/// ``FlowResult/canceled(_:)``. Do not create an app session from controller success; use `onFinish` or
/// `onNativeAction` when applicable.
///
...
public protocol EliteFlowController: Sendable {
    ...
    ///
    /// A success result means a hosted terminal event handler completed. It can follow hosted `onFinish`,
    /// `onNativeAction`, `onError`, or `onClose`; it does not mean hosted authentication succeeded.
