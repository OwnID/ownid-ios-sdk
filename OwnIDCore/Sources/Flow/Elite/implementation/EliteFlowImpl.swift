...
private actor EliteFlowActor {
        ...
        case (.active(let controller, _), .webBridgeOpResult(let operationID, let result)):
            ...
            case .canceled(let reason):
                await userJourney?.completeOperation(
                    ...
                    errorCode: .aborted,
                    source: "EliteFlowActor.reduce.webBridgeOpResult.canceled",
                    message: "Canceled with reason: \(reason.description)"
                )
                return .completed(result: .canceled(reason))
            case .failure(let failure):
                await userJourney?.completeOperation(
                    ...
                    source: "EliteFlowActor.reduce.webBridgeOpResult.failure",
    ...
    private func handleStart(context: EliteFlowContext) async throws -> State {
        ...
        let webBridgeController = webBridgeOperation.startWebBridge(
        ...
        await userJourney?.startOperation(operationID: webBridgeController.operationID)

        taskScope.spawn { [actor = self, operationID = webBridgeController.operationID] in
            let res = await webBridgeController.whenSettled()
