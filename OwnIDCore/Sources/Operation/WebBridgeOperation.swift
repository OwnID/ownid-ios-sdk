...
{
    /// Starts WebBridge with its operation-specific controller type.
    ///
    /// Keep Elite/UI call sites on this typed entry point instead of calling ``OperationCapability/start(params:)`` and
    /// downcasting the returned generic controller. The iOS 13/14 compatibility path uses a back-deployed Swift runtime,
    /// so avoid dynamic casts between constrained operation-controller existentials there.
