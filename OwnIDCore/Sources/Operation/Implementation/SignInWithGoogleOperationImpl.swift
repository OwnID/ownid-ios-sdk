...
internal final class SignInWithGoogleOperationImpl: SignInWithGoogleOperation, @unchecked Sendable {
    enum Event: Sendable {
        case start(SignInWithGoogleOperationParams?)
        case signIn(SocialResult)
        case abort(Reason)
        case complete(OperationResult<AccessTokenWithUserInfo, SignInWithGoogleOperationFailure>)
    }
    ...
    private let ui: any SignInWithGoogleUI
                ...
                case .start(let params):
                    ...
                    if case .created = state {
                        ...
                        taskScope.spawn { [weak self] in
                            guard let self else { return }
                            let params = params ?? SignInWithGoogleOperationParams()
                            ...
                            case .failure(let apiError):
                                ...
                                await self.stream.yield(.complete(.failure(apiError.toGoogleOperationFailure())))
                            case .canceled:
                                guard await self.isAlive() else { return }
                                ...
                                await self.stream.yield(
                                    .complete(
                                        apiResult.fold(
                                            ...
                                            onError: { apiError in .failure(apiError.toGoogleOperationFailure()) },
                        ...
                        case .fail(let error):
                            taskScope.spawn { [weak self] in
                                ...
                                await self.stream.yield(
                                    .complete(
                                        .failure(
                                            SignInWithGoogleOperationFailure.integration(
                                                .providerFailed(
    ...
    @discardableResult
    internal func start(params: SignInWithGoogleOperationParams? = nil) -> SignInWithGoogleOperationController {
    ...
    internal func availability(params: (any CapabilityParams)?) async -> Availability {
        ...
        if let params, !(params is SignInWithGoogleOperationParams) {
            return .unavailable("Unsupported params type: \(String(describing: type(of: params)))")
        }
        return .available
    }

    private func unexpectedEventError(eventDescription: String, state: SignInWithGoogleOperationState) -> SignInWithGoogleOperationFailure {
    ...
    @MainActor
    private func markCompletedIfNeeded(_ result: OperationResult<AccessTokenWithUserInfo, SignInWithGoogleOperationFailure>) -> Bool {
...
extension SignInWithGoogleOperationImpl {

    internal static func create(resolver: any DIContainerResolver) -> any SignInWithGoogleOperation {
        do {
            let resolverWithContext = (resolver as! any DIContainer).withContext("SignInWithGoogleOperation") { _ in }
            return SignInWithGoogleOperationImpl(
                ...
                ui: try resolver.getOrThrow(type: (any SignInWithGoogleUI).self),
                ...
                unsatisfiedDependencies: resolver.getUnsatisfiedDependencies(for: (any SignInWithGoogleOperation).self)
            )
        } catch {
...
private final class Failed: SignInWithGoogleOperation, @unchecked Sendable {
    let operationType: OperationType = .oidcAuthenticationGoogle
    let operationID: OperationID = OperationType.oidcAuthenticationGoogle.createOperationID()
    let controller: SignInWithGoogleOperationController
    private let controllerImpl: OperationControllerImpl<AccessTokenWithUserInfo, SignInWithGoogleOperationFailure>
    private let failure: SignInWithGoogleOperationFailure

    init(error: any Error) {
        let controller = OperationControllerImpl<AccessTokenWithUserInfo, SignInWithGoogleOperationFailure>(operationID: operationID) { _ in
        }
        let failure = SignInWithGoogleOperationFailure.unexpected(
            message: String(describing: error),
    ...
    @discardableResult
    func start(params: SignInWithGoogleOperationParams? = nil) -> SignInWithGoogleOperationController {
        controller
    }
    ...
    @MainActor
    func stateStream() -> AsyncStream<SignInWithGoogleOperationState> {
...
extension OIDCStartAPIFailure {
    fileprivate func toGoogleOperationFailure() -> SignInWithGoogleOperationFailure {
        switch self {
