...
internal final class PhoneVerificationOperationImpl: PhoneVerificationOperation, @unchecked Sendable {

    enum Event: Sendable {
        enum UIEvent: Sendable { case codeEntered(String), resend, cancel, notYou }
        enum APIEvent: Sendable {
            case start(APIResult<any PhoneVerificationAPIController, PhoneVerificationStartAPIFailure>),
                complete(APIResult<AccessOrProofToken, PhoneVerificationCompleteAPIFailure>),
                resend(APIResult<Void, PhoneVerificationResendAPIFailure>),
                cancel(Reason)
        }
        case start(PhoneVerificationOperationParams?)
        case api(APIEvent)
        case ui(UIEvent)
        case errorStringsUpdated(ErrorStrings)
        case abort(Reason)
        case complete(OperationResult<AccessOrProofToken, PhoneVerificationOperationFailure>)
    }
    ...
    private let ui: any PhoneVerificationUI
    private let api: any PhoneVerificationAPI
    ...
    internal init(
        ...
        ui: any PhoneVerificationUI,
        api: any PhoneVerificationAPI,
        taskScope: TaskScope,
            ...
            for await event in stream.sequence {
                ...
                case .start(let params):
                    ...
                    if case .created = state {
                        ...
                        let resolvedParams = params ?? PhoneVerificationOperationParams()
                        ...
                        if let provided = resolvedParams.loginID {
                            ...
                            do {
                                ...
                                if accessToken == nil {
                                    ...
                                    let failure = contextError.toPhoneVerificationOperationFailure()
                        ...
                        if !loginID.isSupportedVerificationTarget(.phoneNumber, loginIDHintID: resolvedParams.loginIDHintID) {
                            await self.stream.yield(
                                .complete(
                                    .failure(
                                        .input(
                                            .unsupportedLoginIDType(
                                                ...
                                                message: "LoginIDType.PhoneNumber or LoginIDType.UserName with loginIDHintID required"
                                            )
                                        )
                                    )
                                )
                        ...
                        let paramsWithLoginID = PhoneVerificationOperationParams(
                            loginID: loginID,
                        ...
                        case .preparing(let params):
                            ...
                            case .success(let apiController):
                                ...
                                if apiController.challenge.methods.otp == nil {
                                    ...
                                    let uiState = PhoneVerificationUIState(
                        ...
                        case .active(let uiState, let apiController):
                            ...
                            case .failure(let apiError):
                                ...
                                if case .badRequest(.wrongCode(_, _, _)) = apiError {
                                    let newState = PhoneVerificationUIState(
                ...
                case .ui(let uiEvent):
                    ...
                    case .active(let uiState, let apiController):
                        ...
                        let busy = PhoneVerificationUIState(
                            challenge: uiState.challenge,
                            isBusy: true,
                            error: nil,
                            onCodeEntered: uiState.onCodeEntered,
    ...
    @discardableResult
    internal func start(
        params: PhoneVerificationOperationParams? = nil
    ) -> any OperationController<AccessOrProofToken, PhoneVerificationOperationFailure> {
    ...
    internal func availability(params: (any CapabilityParams)?) async -> Availability {
        ...
        let operationParams: PhoneVerificationOperationParams?
        if let params {
            guard let typedParams = params as? PhoneVerificationOperationParams else {
        ...
        if !loginID.isSupportedVerificationTarget(.phoneNumber, loginIDHintID: operationParams?.loginIDHintID) {
            return .unavailable("LoginIDType.PhoneNumber or LoginIDType.UserName with loginIDHintID required")
        }
        return .available
    }

    private func unexpectedEventError(eventDescription: String, state: PhoneVerificationOperationState) -> PhoneVerificationOperationFailure
    {
    ...
    @MainActor
    private func markCompletedIfNeeded(_ result: OperationResult<AccessOrProofToken, PhoneVerificationOperationFailure>) -> Bool {
...
extension PhoneVerificationOperationImpl {

    internal static func create(resolver: any DIContainerResolver) -> any PhoneVerificationOperation {
        do {
            let resolverWithContext = (resolver as! any DIContainer).withContext("PhoneVerificationOperation") { _ in }
            return PhoneVerificationOperationImpl(
                ...
                ui: try resolver.getOrThrow(type: (any PhoneVerificationUI).self),
                api: try resolverWithContext.getOrThrow(type: (any PhoneVerificationAPI).self),
                ...
                unsatisfiedDependencies: resolver.getUnsatisfiedDependencies(for: (any PhoneVerificationOperation).self)
            )
        } catch {
...
private final class Failed: PhoneVerificationOperation, @unchecked Sendable {
    let operationType: OperationType = .phoneNumberVerification
    let operationID: OperationID = OperationType.phoneNumberVerification.createOperationID()
    private let controllerImpl: PhoneVerificationOperationControllerImpl
    private let failure: PhoneVerificationOperationFailure

    init(error: any Error) {
        let failure = PhoneVerificationOperationFailure.unexpected(
            message: String(describing: error),
            underlyingError: error.asSendableError()
        )
        let controller = PhoneVerificationOperationControllerImpl(
...
internal final class PhoneVerificationOperationControllerImpl:
    OperationControllerImpl<AccessOrProofToken, PhoneVerificationOperationFailure>, PhoneVerificationOperationController,
    @unchecked Sendable
{
    @MainActor @BroadcastedState private var currentState: PhoneVerificationOperationState = .created

    internal init(
        operationID: OperationID,
        onUserAborted: @escaping @Sendable (Reason) -> Void,
        initialState: PhoneVerificationOperationState = .created
    ) {
    ...
    @MainActor internal var state: PhoneVerificationOperationState {
        get { currentState }
...
extension LoginIDResolutionError {
    fileprivate func toPhoneVerificationOperationFailure() -> PhoneVerificationOperationFailure {
        switch self {
...
extension PhoneVerificationStartAPIFailure {
    fileprivate func toOperationFailure() -> PhoneVerificationOperationFailure {
        switch self {
...
extension PhoneVerificationCompleteAPIFailure {
    fileprivate func toOperationFailure() -> PhoneVerificationOperationFailure {
        switch self {
        case .badRequest(.invalidArgument(let errorCode, let message)),
            .badRequest(.unknown(let errorCode, let message)):
...
extension PhoneVerificationResendAPIFailure {
    fileprivate func toOperationFailure() -> PhoneVerificationOperationFailure {
        switch self {
        case .badRequest(.invalidArgument(let errorCode, let message)),
            .badRequest(.unknown(let errorCode, let message)):
