...
/// Instance-scoped runtime for one ``PasskeyAssertionOperation`` lifecycle.
///
...
internal final class PasskeyAssertionOperationImpl: PasskeyAssertionOperation, @unchecked Sendable {
    enum Event: Sendable {
        case start(PasskeyAssertionOperationParams?)
        case ui(PasskeyResult<AssertionResult>)
        case abort(Reason)
        case complete(OperationResult<AccessToken, PasskeyAssertionOperationFailure>)
    }
    ...
    private let ui: any PasskeyAssertionUI
    private let api: any PasskeyAssertionAPI
    ...
    internal init(
        ...
        ui: any PasskeyAssertionUI,
        api: any PasskeyAssertionAPI,
        taskScope: TaskScope,
        context: Context?,
                ...
                case .start(let params):
                    ...
                    if case .created = state {
                        ...
                        taskScope.spawn { [weak self] in
                            guard let self else { return }
                            let params = params ?? PasskeyAssertionOperationParams()
                            ...
                            if let provided = params.loginID {
                                ...
                                do {
                                    ...
                                    if accessToken == nil {
                                        let failure = (error as! LoginIDResolutionError).toPasskeyAssertionOperationFailure()
                            ...
                            if loginID == nil && accessToken == nil {
                                ...
                                await self.stream.yield(
                                    .complete(
                                        .failure(
                                            PasskeyAssertionOperationFailure.input(
                            ...
                            case .success(let apiController):
                                guard await self.isAlive() else { return }
                                if let timeout = apiController.assertionOptions.timeout {
                                ...
                                self.taskRefs.replace(
                                    ...
                                    with: taskScope.spawnOnMain { [weak self] in
                                        guard let self else { return }
                                        guard await self.isAlive() else { return }
                                        let assertionResult = await ui.getCredential(options: apiController.assertionOptions)
                                        await self.stream.yield(.ui(assertionResult))
                                    }
                                )
                ...
                case .ui(let assertionResult):
                    ...
                    case .active(let apiController):
                        ...
                        case .success(let assertion):
                            taskScope.spawn { [weak self] in
                                guard let self else { return }
                                let verifyResult = await apiController.verify(assertionResult: assertion)
                                guard await self.isAlive() else { return }
                                await self.stream.yield(
                                    .complete(
                                        verifyResult.fold(
                        ...
                        case .failure(let passkeyError):
                            taskScope.spawn { [weak self] in
                                ...
                                let apiResult = await apiController.cancel(
                                    reason: .systemError(details: "Passkey assertion failed: \(passkeyError)")
                                )

                                let operationFailure: PasskeyAssertionOperationFailure = {
                                    switch passkeyError {
                                    case .general(_, let error, _):
                                        let cause = PasskeyAssertionOperationFailureCause.passkeyProvider(underlyingError: error)
                                        return .integration(
                                            .providerFailed(
                                    ...
                                    case .passkeysNoCredential(_, let error, _):
                                        let cause = PasskeyAssertionOperationFailureCause.noCredential(underlyingError: error)
                                        return .credential(
                                            .noApplicablePasskeys(
                                                errorCode: .noApplicablePasskeys,
                                                message: passkeyError.description,
                                                underlyingError: cause
                                            )
                                        )
                                ...
                                await self.stream.yield(.complete(.failure(operationFailure)))
                            }
                        }
    ...
    @discardableResult
    internal func start(params: PasskeyAssertionOperationParams? = nil) -> PasskeyAssertionOperationController {
    ...
    internal func availability(params: (any CapabilityParams)?) async -> Availability {
        ...
        let operationParams: PasskeyAssertionOperationParams?
        if let params {
            guard let typedParams = params as? PasskeyAssertionOperationParams else {
    ...
    private func unexpectedEventError(eventDescription: String, state: PasskeyAssertionOperationState) -> PasskeyAssertionOperationFailure {
    ...
    @MainActor
    private func markCompletedIfNeeded(_ result: OperationResult<AccessToken, PasskeyAssertionOperationFailure>) -> Bool {
...
extension PasskeyAssertionOperationImpl {

    internal static func create(resolver: any DIContainerResolver) -> any PasskeyAssertionOperation {
        do {
            let resolverWithContext = (resolver as! any DIContainer).withContext("PasskeyAssertionOperation") { _ in }
            return PasskeyAssertionOperationImpl(
                ...
                ui: try resolver.getOrThrow(type: (any PasskeyAssertionUI).self),
                api: try resolverWithContext.getOrThrow(type: (any PasskeyAssertionAPI).self),
                ...
                unsatisfiedDependencies: resolver.getUnsatisfiedDependencies(for: (any PasskeyAssertionOperation).self)
            )
        } catch {
...
///
/// Starting this object returns a controller that is already completed with ``PasskeyAssertionOperationFailure``, does
/// not register in ``OperationRegistry``, and reports unavailable with the same diagnostic message.
private final class Failed: PasskeyAssertionOperation, @unchecked Sendable {
    let operationType: OperationType = .passkeyAuth
    let operationID: OperationID = OperationType.passkeyAuth.createOperationID()
    let controller: PasskeyAssertionOperationController
    private let controllerImpl: OperationControllerImpl<AccessToken, PasskeyAssertionOperationFailure>
    private let failure: PasskeyAssertionOperationFailure

    init(error: any Error) {
        let controller = OperationControllerImpl<AccessToken, PasskeyAssertionOperationFailure>(operationID: operationID) { _ in }
        let failure = PasskeyAssertionOperationFailure.unexpected(
    ...
    @discardableResult
    func start(params: PasskeyAssertionOperationParams? = nil) -> PasskeyAssertionOperationController {
        controller
    }
    ...
    @MainActor
    func stateStream() -> AsyncStream<PasskeyAssertionOperationState> {
...
extension LoginIDResolutionError {
    fileprivate func toPasskeyAssertionOperationFailure() -> PasskeyAssertionOperationFailure {
        switch self {
...
extension PasskeyAssertionStartAPIFailure {
    fileprivate func toOperationFailure() -> PasskeyAssertionOperationFailure {
        switch self {
...
extension PasskeyAssertionVerifyAPIFailure {
    fileprivate func toOperationFailure() -> PasskeyAssertionOperationFailure {
        switch self {
        case .badRequest(.invalidArgument(let errorCode, let message)), .badRequest(.unknown(let errorCode, let message)):
            return .input(.invalidRequest(errorCode: errorCode, message: message, apiFailure: self))
