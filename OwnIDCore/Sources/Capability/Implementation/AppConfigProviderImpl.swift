...
internal protocol AppConfigPathMonitoring: AnyObject, Sendable {
    func start(queue: DispatchQueue, onAvailable: @escaping @Sendable () -> Void)
    func cancel()
}

private final class NWAppConfigPathMonitor: AppConfigPathMonitoring, @unchecked Sendable {
...
internal actor AppConfigProviderImpl: AppConfigProvider {
    ...
    private let pathMonitorFactory: @Sendable () -> any AppConfigPathMonitoring
    ...
    private var pathMonitor: (any AppConfigPathMonitoring)?
    ...
    init(
        ...
        pathMonitorFactory: @escaping @Sendable () -> any AppConfigPathMonitoring = { NWAppConfigPathMonitor() }
    ) {
        ...
        _ = taskScope.onShutdown { [weak self] in
            Task {
                await self?.shutdownBackgroundWork()
            }
        }
    ...
    private func startIfNeeded() {
        ...
        monitor.start(queue: pathMonitorQueue) { [weak self, taskScope] in
            _ = taskScope.spawn { [weak self] in
    ...
    private func shutdownBackgroundWork() {
        stopConditionReached = true
        retryEnabled = false
        cancelBackgroundWork()
    }
