...
/// ``create(params:)``. If the app's session layer uses blocking work, dispatch that work away from the main actor
/// inside the callback and return only after the app-owned work completes.
public protocol SessionCreate: Capability, Sendable {
    ...
    /// ``SessionOutput``. OwnID calls this method on the main actor and does not move app-owned work to a background
    /// executor for you.
    ///
