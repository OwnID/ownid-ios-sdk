...
/// parameter types. OwnID invokes these callbacks on the main actor. If a callback uses blocking app-owned work, dispatch
/// that work away from the main actor and return only after it completes.
///
...
public struct SessionCreateBuilder {
    ...
    /// The app owns the failure value. OwnID does not move app-owned work to a background executor for you.
...
/// unavailable for other parameter types. OwnID invokes these callbacks on the main actor. If a callback uses blocking
/// app-owned work, dispatch that work away from the main actor and return only after it completes.
///
/// When ``isAvailable(_:)`` returns `false`, OwnID treats this provider as unavailable for the requested parameters.
