...
/// typed ``APIResult``.
///
/// Keep the controller strongly referenced while the challenge can still complete, resend, or cancel. Releasing or
/// clearing the controller reference only loses the app's handle; it does not cancel the server challenge. Call
/// ``cancel(reason:)`` before clearing the last reference when the app abandons this challenge.
///
...
public protocol EmailVerificationAPIController: Sendable {
    ...
    ///   failure, or ``APIResult/canceled`` if the surrounding task is canceled first. Canceling the Swift task or
    ///   releasing the controller does not cancel the server-side challenge; call this method when the app wants to
    ///   request challenge cancellation.
    ///
