...
/// Starts login for a user that already has an OwnID access token.
///
/// Use this direct API after the app or an OwnID flow has obtained an ``AccessToken``. The token is taken from
/// ``LoginAPIParams/accessToken``, or from the scoped ``Context`` when the parameter is omitted.
///
...
public protocol LoginAPI: APICapability {
    /// Starts token-backed login.
    ///
    /// - Parameter params: Optional parameters. Omit only when the scoped ``Context`` contains the access token to use.
...
/// Parameters for token-backed login.
///
/// - Parameters:
///   - accessToken: Access token proving login ID ownership. The app owns this sensitive value. When `nil`, the SDK
///     uses the access token from the scoped ``Context`` if one is available. Calling login without a resolvable access
///     token completes with ``LoginAPIFailure/badRequest(_:)``.
///
/// OpenAPI source: `LoginRequest` schema.
public struct LoginAPIParams: Sendable {
    /// Access token proving login ID ownership.
    public let accessToken: AccessToken?
    internal let traceParent: String?

    public init(accessToken: AccessToken? = nil) {
        self.init(accessToken: accessToken, traceParent: nil)
    }
...
/// Native failure hierarchy returned by ``LoginAPI/start(params:)``.
///
/// Direct cases correspond to OpenAPI error response components for the `login` operation, plus
/// ``LoginAPIFailure/unexpected(errorCode:message:underlyingError:)`` for SDK-side failures.
///
/// OpenAPI source: `login` operation.
public enum LoginAPIFailure: APIFailure, CustomStringConvertible {
    /// Bad request.
    ///
