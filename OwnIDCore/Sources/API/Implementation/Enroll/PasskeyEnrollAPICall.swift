...
internal final class PasskeyEnrollAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("passkeys/attestation/enroll"))

        let requestBody = InternalAttestationEnrollRequest(proofToken: proofToken.token)
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<Void, PasskeyEnrollAPIFailure> {
    ...
    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> PasskeyEnrollAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadEnrollRequestErrorResponse.self,
                unexpected: PasskeyEnrollAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        case 404:
            do {
                ...
                return failResponse.toUnexpectedFailure(PasskeyEnrollAPIFailure.unexpected, error: error)
            }
        ...
        default:
            return failResponse.toUnexpectedFailure(PasskeyEnrollAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> PasskeyEnrollAPIFailure {
        failResponse.toUnexpectedFailure(PasskeyEnrollAPIFailure.unexpected)
    }
}
