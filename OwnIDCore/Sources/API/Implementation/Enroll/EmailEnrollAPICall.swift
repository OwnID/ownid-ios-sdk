...
internal final class EmailEnrollAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("verifications/email/enroll"))
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<Void, EmailEnrollAPIFailure> {
    ...
    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> EmailEnrollAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadEnrollRequestErrorResponse.self,
                unexpected: EmailEnrollAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        case 404:
            do {
                ...
                return failResponse.toUnexpectedFailure(EmailEnrollAPIFailure.unexpected, error: error)
            }
        ...
        default:
            return failResponse.toUnexpectedFailure(EmailEnrollAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> EmailEnrollAPIFailure {
        failResponse.toUnexpectedFailure(EmailEnrollAPIFailure.unexpected)
    }
}
