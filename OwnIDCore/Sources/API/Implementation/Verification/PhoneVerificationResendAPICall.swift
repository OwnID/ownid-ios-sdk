...
internal final class PhoneVerificationResendAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("verifications/phone/resend"))
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<Void, PhoneVerificationResendAPIFailure> {
        guard successResponse.code == 204 else { return .failure(mapUnhandled(successResponse.toUnexpectedStatusFail())) }
        return .success(())
    }

    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> PhoneVerificationResendAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadResendRequestErrorResponse.self,
                unexpected: PhoneVerificationResendAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        default:
            return failResponse.toUnexpectedFailure(PhoneVerificationResendAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> PhoneVerificationResendAPIFailure {
        failResponse.toUnexpectedFailure(PhoneVerificationResendAPIFailure.unexpected)
    }
}
