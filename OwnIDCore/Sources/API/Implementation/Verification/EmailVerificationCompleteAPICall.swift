...
internal final class EmailVerificationCompleteAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("verifications/email/complete"))
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<AccessOrProofToken, EmailVerificationCompleteAPIFailure> {
    ...
    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> EmailVerificationCompleteAPIFailure {
        ...
        case 400:
            return failResponse.mapDecodedBody(
                ...
                unexpected: EmailVerificationCompleteAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        default:
            return failResponse.toUnexpectedFailure(EmailVerificationCompleteAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> EmailVerificationCompleteAPIFailure {
        failResponse.toUnexpectedFailure(EmailVerificationCompleteAPIFailure.unexpected)
    }
}
