...
internal final class EmailVerificationCancelAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("verifications/email/cancel"))
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<Void, EmailVerificationCancelAPIFailure> {
        guard successResponse.code == 204 else { return .failure(mapUnhandled(successResponse.toUnexpectedStatusFail())) }
        return .success(())
    }

    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> EmailVerificationCancelAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadChallengeRequestErrorResponse.self,
                unexpected: EmailVerificationCancelAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        default:
            return failResponse.toUnexpectedFailure(EmailVerificationCancelAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> EmailVerificationCancelAPIFailure {
        failResponse.toUnexpectedFailure(EmailVerificationCancelAPIFailure.unexpected)
    }
