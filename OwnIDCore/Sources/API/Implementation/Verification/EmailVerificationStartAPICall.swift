...
internal final class EmailVerificationStartAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("verifications/email/start"))
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<VerificationChallenge, EmailVerificationStartAPIFailure> {
    ...
    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> EmailVerificationStartAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadVerificationRequestErrorResponse.self,
                unexpected: EmailVerificationStartAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        case 404:
            do {
                ...
                return failResponse.toUnexpectedFailure(EmailVerificationStartAPIFailure.unexpected, error: error)
            }
        ...
        default:
            return failResponse.toUnexpectedFailure(EmailVerificationStartAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> EmailVerificationStartAPIFailure {
        failResponse.toUnexpectedFailure(EmailVerificationStartAPIFailure.unexpected)
    }
}
