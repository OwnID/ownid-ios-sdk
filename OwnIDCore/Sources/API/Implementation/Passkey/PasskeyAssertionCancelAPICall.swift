...
internal final class PasskeyAssertionCancelAPICall: APICall {
    ...
    internal init(
        ...
        var request = NetworkRequest(url: apiBaseURL.appendingPathComponent("passkeys/assertion/cancel"))
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<Void, PasskeyAssertionCancelAPIFailure> {
        guard successResponse.code == 204 else { return .failure(mapUnhandled(successResponse.toUnexpectedStatusFail())) }
        return .success(())
    }

    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> PasskeyAssertionCancelAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadChallengeRequestErrorResponse.self,
                unexpected: PasskeyAssertionCancelAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        default:
            return failResponse.toUnexpectedFailure(PasskeyAssertionCancelAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> PasskeyAssertionCancelAPIFailure {
        failResponse.toUnexpectedFailure(PasskeyAssertionCancelAPIFailure.unexpected)
    }
}
