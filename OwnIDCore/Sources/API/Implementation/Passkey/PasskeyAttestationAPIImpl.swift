...
internal final class PasskeyAttestationAPIImpl: PasskeyAttestationAPI {
    ...
    internal func start(
        ...
        do {
            ...
            let request = try PasskeyAttestationOptionsAPICall(
                apiBaseURL: baseUrl,
                coder: coder,
                loginID: loginID,
                accountDisplayName: params?.accountDisplayName ?? context?.accountDisplayName,
                accessToken: accessToken,
                traceParent: traceParent
            )
            return try await request.executeWithInterceptor(network: network, interceptor: interceptor)
                .map { attestationOptions in
                    PasskeyAttestationAPIControllerImpl(
                        apiBaseURL: baseUrl,
                        network: network,
                        coder: coder,
    ...
    internal static func create(resolver: any DIContainerResolver) -> any PasskeyAttestationAPI {
        do {
            return PasskeyAttestationAPIImpl(
...
private final class Failed: PasskeyAttestationAPI, @unchecked Sendable {
...
extension PasskeyAttestationStartAPIFailure {
    fileprivate static func contextLoginIDFailure(_ error: LoginIDResolutionError) -> PasskeyAttestationStartAPIFailure {
        switch error {
