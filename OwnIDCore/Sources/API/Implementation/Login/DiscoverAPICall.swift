...
internal final class DiscoverAPICall: APICall {
    ...
    func mapHttpSuccess(_ successResponse: NetworkResponse.Success) -> APIResult<LoginResponse, DiscoverAPIFailure> {
        do {
                    ...
                    return .success(
                        .authRequired(
                            LoginResponse.AuthRequired(
                                authRequirements: Self.mapAuthRequirements(authRequirements),
    ...
    func mapHttpError(_ failResponse: NetworkResponse.Fail.HttpError) -> DiscoverAPIFailure {
        switch failResponse.statusCode {
        case 400:
            return failResponse.mapDecodedBody(
                coder: coder,
                as: InternalBadLoginIdRequestErrorResponse.self,
                unexpected: DiscoverAPIFailure.unexpected
            ) { error in
                error.toFailure(
        ...
        default:
            return failResponse.toUnexpectedFailure(DiscoverAPIFailure.unexpected)
        }
    }

    func mapUnhandled(_ failResponse: NetworkResponse.Fail) -> DiscoverAPIFailure {
        failResponse.toUnexpectedFailure(DiscoverAPIFailure.unexpected)
    }

    private static func mapAuthRequirements(_ authRequirements: InternalAuthRequirements) -> AuthRequirements {
