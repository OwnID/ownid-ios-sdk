...
internal final class DiscoverAPIImpl: DiscoverAPI {
    ...
    func start(params: DiscoverAPIParams?) async -> APIResult<LoginResponse, DiscoverAPIFailure> {
        do {
            ...
            guard let loginID else {
                return .failure(.badRequest(.invalidArgument(errorCode: .invalidArgument, message: "LoginID is required")))
            }

            let request = try DiscoverAPICall(
            ...
            case .loginIDTypeNotSupported(_, let message):
                return .failure(.badRequest(.unsupportedLoginIDType(errorCode: error.errorCode, message: message)))
    ...
    internal static func create(resolver: any DIContainerResolver) -> any DiscoverAPI {
        do {
            return DiscoverAPIImpl(
...
private final class Failed: DiscoverAPI, @unchecked Sendable {
