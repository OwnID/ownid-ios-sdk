...
// Covers: UI-PRESENT-210, UI-PRESENT-250
@MainActor
@Suite(.serialized)
struct LoginIDCollectDefaultUIRuntimeTests {
    ...
    @Test func `Login ID field requests initial focus when presentation is ready`() async throws {
        ...
        let host = SwiftUIRuntimeHost(
            rootView: LoginIDCollectRuntimeFixture(isReadyForInitialFocus: true)
        )
        defer { host.close() }

        let textField = try await waitForFocusedTextField(in: host)
        #expect(textField.accessibilityLabel == "Email")
    }
...
private struct LoginIDCollectRuntimeFixture: View {
    ...
    init(
        ...
        isReadyForInitialFocus: Bool = false
    ) {
...
@MainActor
private func waitForFocusedTextField<Content: View>(
    in host: SwiftUIRuntimeHost<Content>
) async throws -> UITextField {
    for _ in 0..<200 {
        await host.settle(cycles: 1)
        if let textField = host.textFields().first(where: { $0.isFirstResponder }) {
            return textField
        }
        try await Task.sleep(nanoseconds: 10_000_000)
    }

    let focusedTextField = host.textFields().first(where: { $0.isFirstResponder })
    return try #require(focusedTextField, "Expected Login ID text field to become first responder")
}
