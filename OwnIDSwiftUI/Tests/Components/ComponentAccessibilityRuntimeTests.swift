...
// Covers: UI-PRESENT-150, UI-PRESENT-270
@MainActor
@Suite(.serialized)
struct ComponentAccessibilityRuntimeTests {
    ...
    @Test func `Default widget components preserve semantics under injectable Dynamic Type`() throws {
        ...
        #expect(element.accessibilityTraits.contains(UIAccessibilityTraits.notEnabled) == false)

        try activateControl(labeled: "Skip password", in: host)
