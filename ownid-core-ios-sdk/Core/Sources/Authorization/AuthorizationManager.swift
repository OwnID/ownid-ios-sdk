...
extension OwnID.CoreSDK.AccountManager {
    ...
    static var defaultAccountManager: CreationClosure {
        { store, domain, challenge, browserBaseURL in
            ...
            let current = Self { credsIds in
                if #available(iOS 16.0, *) {
                    manager.signIn(credsIds: credsIds)
                }
            } cancelClosure: {
                if #available(iOS 16.0, *) {
                    manager.cancel()
                }
            } signUpClosure: { userName, credsIds in
                if #available(iOS 16.0, *) {
                    manager.signUpWith(userName: userName, credsIds: credsIds)
                }
            }
            return current
        }
...
extension OwnID.CoreSDK {
    struct AccountManager {
        var signInClosure: (_ credsIds: [String]) -> Void
        var cancelClosure: () -> Void
        var signUpClosure: (_ userName: String, _ credsIds: [String]) -> Void
        
        func signIn(credsIds: [String]) {
            signInClosure(credsIds)
        }
        ...
        func signUpWith(userName: String, credsIds: [String]) {
            signUpClosure(userName, credsIds)
        }
    }
}
