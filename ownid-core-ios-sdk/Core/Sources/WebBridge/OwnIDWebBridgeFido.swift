...
extension OwnID.CoreSDK {
    final class WebBridgeFido: NamespaceHandler {
        ...
        private func fidoError(message: String = OwnID.CoreSDK.ErrorMessage.dataIsMissingError())
            -> CoreViewModel.FidoErrorRequestBody.Error
        {
            return CoreViewModel.FidoErrorRequestBody.Error(
                name: message,
                type: message,
                message: message
            )
        ...
        func invoke(
            bridgeContext: WebBridgeContext,
            action: String,
            params: String,
            metadata: JSMetadata?,
            ...
            case "create", "get":
                ...
                guard let jsonData = params.data(using: .utf8),
                    ...
                    let context = paramsJson["context"] as? String
                else {
                ...
                guard let jsonData = params.data(using: .utf8),
                    let fidoData = try? JSONDecoder().decode(CoreViewModel.FidoStepData.self, from: jsonData)
                else {
        ...
        private func isOriginAllowed(_ allowedOriginRules: [URL], sourceOrigin: URL?) -> Bool {
            ...
            let allowedOrigin = allowedOriginRules.first { rule in
                if let sourceHost = sourceOrigin?.host, let allowHost = rule.host, let sourceScheme = sourceOrigin?.scheme,
                    let allowScheme = rule.scheme
                {
        ...
        private func enroll(
            ...
            guard let jsonData = params.data(using: .utf8),
                let fidoData = try? JSONDecoder().decode(EnrollManager.FIDOCreateModel.self, from: jsonData)
            else {
                completion(handleErrorResult(fidoError: fidoError()))
                return
            }
            ...
            authManager = OwnID.CoreSDK.AuthManager(
                store: store,
        ...
        private func reducer(isEnroll: Bool, completion: @escaping (_ result: String) -> Void) -> (
            inout AuthManager.State, AuthManager.Action
        ) -> [Effect<AuthManager.Action>] {
            let reducer: (inout AuthManager.State, AuthManager.Action) -> [Effect<AuthManager.Action>] = { [weak self] state, action in
                ...
                case .didFinishRegistration(let fido2RegisterPayload):
                    if isEnroll {
                        let response = EnrollManager.ResultRequestBodyResponse(
                            clientDataJSON: fido2RegisterPayload.clientDataJSON,
                            attestationObject: fido2RegisterPayload.attestationObject
                        )
                        let model = EnrollManager.ResultRequestBody(
                            id: fido2RegisterPayload.credentialId,
                            type: .publicKey,
                            response: response
                        )
        ...
        private func error(_ error: AuthManager.AuthManagerError) -> CoreViewModel.FidoErrorRequestBody.Error {
            ...
            case .authManagerAuthError(let error), .authManagerGeneralError(let error):
                ...
                fidoError = CoreViewModel.FidoErrorRequestBody.Error(
        ...
        private func handleErrorResult(fidoError: CoreViewModel.FidoErrorRequestBody.Error) -> String {
            let error = JSError(
                error: OwnID.CoreSDK.JSErrorData(name: self.name.rawValue, type: fidoError.type, message: fidoError.message)
            )
