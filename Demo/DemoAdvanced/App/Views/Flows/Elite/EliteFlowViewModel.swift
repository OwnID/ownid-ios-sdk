...
final class EliteFlowViewModel: ObservableObject {
    ...
    func startEliteFlow() {
        ...
        Task { @MainActor [weak self] in
            let flowContext = EliteFlowContext { builder in
                ...
                builder.events { events in
                    events.onNativeAction { [weak self] loginID, ownIdData, accessToken in
                        self?.log.add(
                            "Elite Flow: onNativeAction: \(loginID), ownIdData=\(ownIdData ?? "nil"), accessToken=\(Self.describe(accessToken))"
                        )
            ...
            await controller.whenSettled()
                .onSuccess { self?.log.add("Elite Flow settled") }
    ...
    func completeRegistration(name: String) async {
        ...
        do {
            try await DemoAuthIntegrationProvider.integration.registerAndSignIn(
