...
final class HeadlessViewModel: ObservableObject {
    ...
    private func run() async {
        ...
        case .accountNotFound(let accountNotFound):
            ...
            if await registerDemoUser(email: email) {
    ...
    private func runPasskeyAuthentication(headless: OwnIDHeadless, onMissingPasskey: () -> Void = {}) async -> AccessToken? {
        ...
        return await passkeyAuth.start().whenSettled()
            ...
            .onError { error in
                log.add("Passkey Authentication failed: \(error)")
    ...
    private func login(accessToken: AccessToken, registerMissingUser: Bool = false) async -> AccessToken? {
        ...
        let response = await headless.auth.login.start()
            .onError { error in log.add("Login failed: \(error)") }
        ...
        case .authRequired(let required):
            log.add("Login: TargetScore not achieved: \(required.reason ?? "unknown")")
            return nil
        case .accountNotFound(let notFound):
            log.add("Login: Account not found: \(notFound.reason ?? "unknown")")
            if registerMissingUser, await registerDemoUser(email: screenState.email) {
                return await login(accessToken: accessToken)
            }
            return nil
        case .accountBlocked(let blocked):
            log.add("Login: Account blocked: \(blocked.reason ?? "unknown")")
            return nil
        }
    }
    ...
    private func passkeyEnroll(_ accessToken: AccessToken) {
        Task { @MainActor [weak self] in
            ...
            await passkeyEnroll.start().whenSettled()
                ...
                .onError { error in self.log.add("Passkey Enroll failed: \(error)") }
        }
    }
                ...
                Task { @MainActor [weak self] in
                    ...
                    await headless.verifications.email.start()
                        ...
                        .onError { error in
                            ...
                            self.viewModel.log.add("Email Verification failed: \(error)")
        ...
        func resend() {
            Task { @MainActor [weak self] in
                ...
                await controller.resend()
                    ...
                    .onError { error in
                        viewModel.log.add("Email Verification resend: \(error)")
                        viewModel.screenState.flowState = .emailVerification(
                            challenge: challenge,
                            resendCount: resendCount,
                            resendAvailableAt: resendAvailableAt,
        ...
        func cancel() {
            Task { @MainActor [weak self] in
                ...
                await controller.cancel(reason: .userClose())
                    ...
                    .onError { error in viewModel.log.add("Email Verification cancellation: \(error)") }
                ...
                await controller.completeWithCode(code: otpCode)
                    ...
                    .onError { error in
                        if case .badRequest(.wrongCode) = error {
                            viewModel.log.add("Email Verification: Wrong code: \(error)")
                            viewModel.screenState.flowState = .emailVerification(
                                challenge: challenge,
                                resendCount: resendCount,
                                resendAvailableAt: resendAvailableAt,
                            ...
                            viewModel.log.add("Email Verification: \(error)")
                            viewModel.screenState.flowState = .failed
                            self.finish(nil)
                        }
                    }
    ...
    private func registerDemoUser(email: String) async -> Bool {
        do {
            _ = try await DemoBaseApp.identityPlatform.register(
                name: email,
                email: email,
                password: "SomeRandomLongAndCrypticPassword",
                ownIdData: nil
            )
            log.add("Registering user: OK")
            return true
        } catch {
            log.add("Registering user failed: \(error)")
            return false
        }
    }
}
