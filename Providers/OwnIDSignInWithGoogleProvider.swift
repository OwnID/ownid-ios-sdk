...
extension OwnIDProvidersRegistrar {
    ...
    /// - Returns ``SocialResult/fail(error:)`` for empty client IDs, missing presenters, missing ID tokens, and
    ///   non-cancellation GoogleSignIn failures.
    ///
    ...
    /// - A new sign-in request is still rejected until the active request finishes.
