...
extension OwnIDProvidersRegistrar {
    mutating func demoIdentityProviders(authIntegration: CustomDemoAuthIntegration) {
        sessionCreate { provider in
            ...
            provider.create { params in
                do {
                    ...
                    authIntegration.save(demoSession)
        ...
        passwordAuthenticate { provider in
            ...
            provider.authenticate { params in
                do {
                    let session = try await authIntegration.authenticateSession(email: params.loginID.id, password: params.password)
                    authIntegration.save(session)
